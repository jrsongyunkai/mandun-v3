      ...
      <el-row>
        <span class="box-right">
          <el-col @click.native="handleSwitch(datas, 'f')" class="pointer grid-content grid-content-open" :class="datas['f-alarmInfo'] ? 'grid-content-close' : ''">
            ...
            <span>
              <p>{{ $t('control.pressure') + '：' + (datas.F ===undefined ?'--'+datas['f-Unit']:Object.keys(datas).length > 0 ? (datas.F + datas['f-Unit']):'--') }}</p>
              <p>{{ $t('control.upperLimit') + '：' + (Object.keys(datas).length > 0 ? (datas.FH + datas['f-Unit']) : '--') }}</p>
              <p>{{ $t('control.lowerLimit') + '：' + (Object.keys(datas).length > 0 ? (datas.FL + datas['f-Unit']) : '--') }}</p>
            </span>
          </el-col>
        </span>
        <span class="box-right">
          <el-col @click.native="handleSwitch(datas, 't')" class="pointer grid-content grid-content-open" :class="datas['t-alarmInfo'] ? 'grid-content-close' : ''">
            ...
            <span>
              <p>{{ $t('menu.temperature') + '：' + (datas.T===undefined ? '--'+datas['t-Unit']: Object.keys(datas).length > 0 ? (datas.T + datas['t-Unit']) : '--') }}</p>
              <p>{{ $t('control.upperLimit') + '：' + (Object.keys(datas).length > 0 ? (datas.TH + datas['t-Unit']) : '--') }}</p>
              <p>{{ $t('control.lowerLimit') + '：' + (Object.keys(datas).length > 0 ? (datas.TL + datas['t-Unit']) : '--') }}</p>
            </span>
          </el-col>
        </span>
      </el-row>
