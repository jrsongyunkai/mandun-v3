...
<script>
import { avgVoltageStats, temperatureWDetail, queryBoxDetails, modifyMacRate } from '@/api/api'
  ...
  data () {
    return {
      ...
      options: {
        ...
        title: {
          show: true,
          text: this.$t('main.allDay') + this.$t('echarts.voltageSituation')
        },
        tooltip: ''
      },
      detailsFlag: false,
  ...
  methods: {
    init () {
      ...
      this.$nextTick(function () {
        ...
        let data2 = []
        let that = this
        avgVoltageStats(params)
          .then(res => {
            res.data.forEach(function (item, index) {
              that.options.xAxis.push(item.time)
              data.push(item.number1)
              data2.push(item.number2)
            })
            this.options.series.push({name: that.$t('echarts.lineVoltage'), type: 'line', data: data, smooth: true, itemStyle: {normal: {color: '#08BEC1'}}, symbol: 'emptyCircle'})
            this.options.series.push({name: that.$t('echarts.phaseVoltage'), type: 'line', data: data2, smooth: false, itemStyle: {normal: {color: '#B8A5DD'}}, symbol: 'emptyCircle'})
            this.flag = true
          })
          .catch(err => {
            if (err) {
    ...
    handleDayPicker (val) {
      ...
      this.date = val
      this.value = val
      this.options.title.text = this.$t('main.allDay') + this.$t('echarts.voltageSituation')
      this.init()
      this.queryList()
    },
    handleMonthPicker (val) {
      ...
      this.date = val
      this.month = val
      this.options.title.text = this.$func.getMonthText(val) + this.$t('echarts.voltageSituation')
      this.init()
      this.queryList()
    },
    handleDatePicker (val) {
      if (val === 'day') {
        ...
        this.options.title.text = this.$func.getMonthText(this.month) + this.$t('echarts.voltageSituation')
      }
      this.datePicker = val
      this.init()
      this.queryList()
