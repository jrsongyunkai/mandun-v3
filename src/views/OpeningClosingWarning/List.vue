<template>
  <div id="openingClosingWarning">
    <el-col>
      ...
      <div :style="alarmTableHeight" v-el_scrollBar>
        <el-table
          :data="tableData"
          size="small"
          ...
          <el-table-column
            align="center"
            :label="$t('table.warning') + $t('table.type')">
            <template slot-scope="scope">
              <span :class="scope.row.type_number === 1003 ? 'c-primary' : 'c-warning'">{{ scope.row.type_number === 1003 ? $t('types.openingW') : $t('types.closingW')}}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="alarmTime"
            align="center"
            :label="$t('table.warning') + $t('dialog.time')">
          </el-table-column>
        </el-table>
      </div>
...
<script>
/* global ctxPaths */
import { projectOpeningAndClosingWarning, queryBoxDetails, modifyMacRate } from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    queryList () {
      ...
      projectOpeningAndClosingWarning(params)
        .then(res => {
          if (res.success) {
    ...
    handleCommand (command) {
      this.$func.windowCountDown(ctxPaths + '/project/main/v3/xls/openingAndClosingWarning.as?projectCode=' + this.$store.state.projectCode + '&mac=' + this.formInline.mac + '&pageNo=' + this.currentPage + '&pageSize=' + this.pageSize + '&startTime=' + this.formInline.rangeDate[0] + '&endTime=' + this.formInline.rangeDate[1] + '&exportType=' + (command === 'current' ? 'exportCurrent' : ''))
    }
  },
  watch: {
...
.el-pagination
  ...
  .el-pagination__sizes, >>> span:not([class*=suffix]), >>> .el-input__inner
    margin-right 0
    height 28px
#openingClosingWarning >>> .details-dialog
