<template>
    <div id="leakageA">
      ...
      <el-col :md="12">
        <line-chart :options="loadOptions" v-if="flags"></line-chart>
      </el-col>
      <el-col>
        <el-table
          :data="tableData"
...
<script>
import { currentStats, loadStats, temperatureWDetail, queryBoxDetails, modifyMacRate } from '@/api/api'
...
export default {
  data () {
    return {
      ...
      options: {
        ...
        title: {
          show: true,
          text: this.$t('main.allDay') + this.$t('echarts.currentSituation')
        }
      },
      loadOptions: {
        xAxis: [],
        ...
        title: {
          show: true,
          text: this.$t('main.allDay') + this.$t('echarts.loadSituation')
        }
      },
      detailsFlag: false,
      detailsObj: {
  ...
  methods: {
    init () {
      ...
      this.loadOptions.series = []
      let params = {
        typeNumbers: '3,4,1008',
      ...
      this.$nextTick(function () {
        ...
        currentStats(params)
          .then(res => {
            ...
            this.options.series.push({name: that.$t('echarts.currentAmpere'), type: 'line', data: data, smooth: true, itemStyle: {normal: {areaStyle: {type: 'default'}, color: '#08BEC1'}}, symbol: 'emptyCircle'})
            this.flag = true
          })
          .catch(err => {
            if (err) {
        ...
        loadStats(params)
          .then(res => {
            res.data.forEach(function (item, index) {
              that.loadOptions.xAxis.push(item.time)
              loadData.push(item.number)
            })
            this.loadOptions.series.push({name: that.$t('echarts.loadKWh'), type: 'line', data: loadData, smooth: true, itemStyle: {normal: {areaStyle: {type: 'default'}, color: '#B8A5DD'}}, symbol: 'emptyCircle'})
            this.flags = true
          })
          .catch(err => {
            if (err) {
    ...
    handleDayPicker (val) {
      ...
      this.date = val
      this.value = val
      this.options.title.text = this.$t('main.allDay') + this.$t('echarts.currentSituation')
      this.loadOptions.title.text = this.$t('main.allDay') + this.$t('echarts.loadSituation')
      this.init()
      this.queryList()
    },
    handleMonthPicker (val) {
      ...
      this.date = val
      this.month = val
      this.options.title.text = this.$func.getMonthText(val) + this.$t('echarts.currentSituation')
      this.loadOptions.title.text = this.$func.getMonthText(val) + this.$t('echarts.loadSituation')
      this.init()
      this.queryList()
    },
    handleDatePicker (val) {
      if (val === 'day') {
        ...
        this.options.title.text = this.$func.getMonthText(this.month) + this.$t('echarts.currentSituation')
        this.loadOptions.title.text = this.$func.getMonthText(this.month) + this.$t('echarts.loadSituation')
      }
      this.datePicker = val
      this.init()
      this.queryList()
