...
export default {
  data () {
    return {
      ...
      options: {
        ...
        title: {
          show: true,
          text: this.$t('main.allDay') + this.$t('echarts.temperatureAlarmSituation')
        }
      },
      detailsFlag: false,
      detailsObj: {
  ...
  methods: {
      ...
      this.$nextTick(function () {
        ...
        temperatureW(params)
          .then(res => {
            ...
            this.options.series.push({name: that.$t('echarts.numberOfTemperatureAlarm'), type: 'line', data: data, smooth: true, itemStyle: {normal: {areaStyle: {type: 'default'}, color: '#d19edc'}}, symbol: 'emptyCircle'})
            this.flag = true
          })
          .catch(err => {
            if (err) {
    ...
    handleDayPicker (val) {
      ...
      this.date = val
      this.value = val
      this.options.title.text = this.$t('main.allDay') + this.$t('echarts.temperatureAlarmSituation')
      this.init()
      this.queryList()
    },
    handleMonthPicker (val) {
      ...
      this.date = val
      this.month = val
      this.options.title.text = this.$func.getMonthText(val) + this.$t('echarts.temperatureAlarmSituation')
      this.init()
      this.queryList()
    },
    handleDatePicker (val) {
      if (val === 'day') {
        ...
        this.options.title.text = this.$func.getMonthText(this.month) + this.$t('echarts.temperatureAlarmSituation')
      }
      this.datePicker = val
      this.init()
      this.queryList()
